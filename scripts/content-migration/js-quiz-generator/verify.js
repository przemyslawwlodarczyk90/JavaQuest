...
for(const f of process.argv.slice(2)){
  ...
  j.exercises.forEach((e,i)=>{if(!e.prompt||!e.hint||!e.solution)bad.push('ex'+i);else{const r=run(e.solution);if(r.err&&!/DOM|document|window|fetch|setInterval|clearInterval|localStorage|sessionStorage|location\.|history\.|console\.(table|time|timeEnd|group|groupEnd|count|assert|trace)/.test(e.solution))bad.push('exerr'+i+':'+r.err)}});
